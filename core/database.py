...
def add_turn(conv_id: int, question: str, verdict: str = None,
             arbiter_role_id: str = None, seq: int = None) -> int:
    ...
    with get_connection() as conn:
        cur = conn.execute(
            """
            INSERT INTO turns
                (conv_id, seq, question, verdict, arbiter_role_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (conv_id, seq, question, verdict, arbiter_role_id, _now()),
        )
        return cur.lastrowid
...
def add_answer(turn_id: int, role_id: str, content: str,
    ...
    with get_connection() as conn:
        cur = conn.execute(
            """
            INSERT INTO answers (turn_id, role_id, content, trajectory, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (turn_id, role_id, content, traj_text, _now()),
        )
        return cur.lastrowid
...
def _fetch_answers(conn, turn_id: int) -> list:
    """内部：取某轮的全部成员输出（含轨迹还原）"""
    rows = conn.execute(
        "SELECT role_id, content, trajectory, created_at FROM answers "
        "WHERE turn_id = ? ORDER BY answer_id ASC",
        (turn_id,),
    ).fetchall()
    result = []
    for r in rows:
...
def save_turn(conv_id: int, question: str, members: list,
              verdict: str = None, arbiter_role_id: str = None) -> int:
