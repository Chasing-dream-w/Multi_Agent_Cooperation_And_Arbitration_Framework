...
> 一个基于 ReAct 范式的多 Agent 协同与仲裁系统，通过“成员独立推理 + 仲裁交叉裁决”机制，降低单模型在复杂问题上的偏差风险。

## 📖 项目简介
本项目手写实现了 ReAct 闭环（不依赖 LangChain），设计成员角色池与仲裁角色池，支持用户指定至少两名成员与一名仲裁者进行交叉裁决。系统内置 11 种工具，支持异常捕获与降级策略，并具备全链路轨迹追踪与持久化存储，支持回放与调试。
...
- 🛠️ **工具调用与容错**：内置 11 种工具，设计异常捕获与降级策略，单点故障时返回缓存或提示信息，避免 Agent 中断。
